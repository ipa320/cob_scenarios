...
__all__ = ['Concurrence2']

class Concurrence2(smach.container.Container):
    ...
    def add(label, state, remapping={}):
        ...
        self = Concurrence2._currently_opened_container()
    ...
    def execute(self, parent_ud = smach.UserData()):
        ...
        while not smach.is_shutdown():
            if all([o is not None for o in self._child_outcomes.values()]):  ### used to be all instead of any
                break
            self._done_cond.acquire()
            self._done_cond.wait()
            self._done_cond.release()
    ...
    def _state_runner(self,label):
        ...
        with self._done_cond:
            ...
            # if preempt_others or all([o is not None for o in self._child_outcomes.values()]):  # used to be all instead of any
            self._done_cond.notify_all()
